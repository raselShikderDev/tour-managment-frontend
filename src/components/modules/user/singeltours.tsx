    ...
    <section className="py-20 container mx-auto">
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-3 font-semibold">Cost: {tour.costForm ? `$${tour.costForm}` : "N/A"}</p>
          <p className="text-sm text-muted-foreground">
            Max Guests: {tour.maxGuest || "N/A"}
          </p>
          <p className="text-sm text-muted-foreground">
            Start Date: {tour.startDate ? new Date(tour.startDate).toLocaleDateString() : "N/A"}
          </p>
          <p className="text-sm text-muted-foreground">
            End Date: {tour.endDate ? new Date(tour.endDate).toLocaleDateString() : "N/A"}
          </p>
          ...
          <div className="mt-4">
            ...
            <ul className="list-disc ml-5">
              {tour.tourPlan?.length ? tour.tourPlan.map((t, idx) => <li key={idx}>{t}</li>) : <li>N/A</li>}
            </ul>
          </div>
        </CardContent>
      </Card>
